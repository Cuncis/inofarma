...
## Known flake: factory-generated codes can collide with seeded demo codes
`Branch::factory()->create()` / `Customer::factory()->create()` / `Product::factory()->create()` occasionally hit a UNIQUE constraint on `code`/`sku` when a test calls `$this->seed()` first — the factory's own sequence isn't aware of codes the demo seeder already used (`CB-001`, `PRD-001`, `CUS-001`, ...), so a collision is possible, not just occasional bad luck from parallel runs. Seen in `PaymentReconciliationTest` on `branches.code`/`customers.code`/`products.sku`, always clears on a bare rerun. Pre-existing, not caused by any one test — if a run fails on a UNIQUE constraint for a factory-created row's code/sku right after `$this->seed()`, rerun before assuming a real regression.
