...
/**
 * Same scenarios as `ProductCsvImportTest`, exercised through the Filament
 * page at /admin/produk/impor instead of the legacy Inertia route.
 */
class ProductImportTest extends TestCase
{
    ...
    private function csvUpload(array $rows): UploadedFile
    {
        ...
        return UploadedFile::fake()->createWithContent('produk.csv', $content);
    }
    ...
    public function test_reimporting_the_same_sku_updates_instead_of_duplicating(): void
    {
        ...
        Livewire::test(ProductImport::class)
            ->callAction('import', data: ['file' => $this->csvUpload([$row])]);
    ...
    public function test_bebas_terbatas_products_import_as_nonaktif_pending_a_p1_p6_warning(): void
    {
        ...
        $this->assertSame(1, $page->instance()->result['warnedInactive']);
    ...
    public function test_a_row_with_no_sku_is_skipped_without_aborting_the_rest(): void
    {
        ...
        $page = Livewire::test(ProductImport::class)
            ->callAction('import', data: ['file' => $this->csvUpload([$badRow, $goodRow])]);

        $this->assertSame(1, $page->instance()->result['created']);
        $this->assertSame(2, $page->instance()->result['failed'][0]['row']);
